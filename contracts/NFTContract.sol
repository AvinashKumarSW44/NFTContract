...
contract NFT is ERC721, Pausable, Ownable, ERC721Burnable, ReentrancyGuard {
    ...
    function sell(uint _tokenID, uint256 _price) external {
    ...
    function buy(uint256 _tokenID, uint256 _price) external nonReentrant() {
        ...
        safeTransferFrom(idToDetail[_tokenID]._sellerAddress, msg.sender,_tokenID);
        amount = ((_price/ 100) * 2);
        _price = _price - amount;   
        // Error in this line we have to figure out this

        IERC20(tokenContract).transfer(idToDetail[_tokenID]._sellerAddress, _price);
        IERC20(tokenContract).transfer(owner(), amount);
           for (uint i = 0; i < unSold.length; i++){
            if(unSold[i] == _tokenID)
            unSold.push(unSold[i]);
    }


        emit buyNFT(idToDetail[_tokenID]._sellerAddress, msg.sender, _price, _tokenID);
